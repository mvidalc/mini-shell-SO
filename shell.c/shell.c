...
char *read_line(char *line){
		...
		strcpy(strprompt,MAGENTA_T"");
		strcat(strprompt,logname);
		strcat(strprompt,MAGENTA_T"@");
		strcat(strprompt,hostname);
		strcat(strprompt,VERDE_T"~");
		strcat(strprompt,cwd);
		strcat(strprompt,PROMPT RESET_COLOR);
	...
	int i = 0;
 	while(args[i]!=NULL){
  		if(strcmp(args[i],">")==0){
   			if(args[i+1]!=NULL){
         	args[i]=NULL;
         	fd = open(args[i+1],O_CREAT | O_RDWR , S_IRUSR | S_IWUSR);
			if(fd!=-1)	return 1;
			}
   			else{ 
